...
# + 숨김파일까지 출력
ls -al
    -t (수정 순서대로 오름차순) // -s (파일용량 큰 순서대로 정렬)
    -r (파일명으로 내림차순으로 정렬) // -i (파일명으로 오름차순으로 정렬)
...
# 절대 경로 폴더로 이동
cd /home/jihee/my_dir  (절대경로 // 최상자부터 시작하기 때문)
...
# home
    (특정 계정의 home 경로: 로그인한 계정의 home) 경로로 이동     **루트랑 헷갈리지 않기
cd
...
# nano 편집기 사용  (안에 들어가서 내용 추가/수정 가능)
nano 파일명
